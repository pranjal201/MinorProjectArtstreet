...
from .models import SalerDetail, Product, ProductSize,  category, Orders,MyCart
		...
		if request.method == 'POST':
			...
			while i <= size_no:
				if request.POST.get(f'size{i}'):
					sizes.append([request.POST.get(f'size{i}')])
                                        #,int(request.POST.get(f'quantity{i}'))
				i+=1
			Product(product_id2=prod_id2,shop=request.user,product_name=prod_name,category=category.objects.get(id=int(cat)),subcategory=subcategory,price=price,desc=desc,gst=gst_l,image1=image1).save()
			...
			p.save()
			for siz in sizes:
				ProductSize(product=p,size=siz[0]).save()
